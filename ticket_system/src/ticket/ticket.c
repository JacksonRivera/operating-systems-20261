...
long long ticket_generate_radicado(void) {
  // Combinación de time(NULL) + rand() como sugiere el enunciado. :contentReference[oaicite:2]{index=2}
  long long now = (long long)time(NULL);     // cast explícito
...
int ticket_write_to_file(const Ticket *t, const char *assets_dir) {
  ...
  // Contenido mínimo del archivo :contentReference[oaicite:4]{index=4}
